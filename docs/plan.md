...
- [x] 2.10 — Home dynamique — *Livré : greeting dynamique depuis `profileStore` (« Hey, Othmane » + fallback neutre), hero « Find someone » premium (accent dégradé proximité statique → reduced-motion safe), entrée Join (code 6 car.), **aperçu des 3 dernières rencontres** depuis `historyStore` (newest-first) **réutilisant la ligne riche de 2.9** (`HistoryRow` extrait, zéro logique dupliquée) + état vide ; tout le statique/placeholder enlevé ; a11y (rôles+labels sur Find/Join/récents, cibles ≥44px) ; navbar intacte. **Pas de map** (phase device), pas de feature V2. Preuves : `pnpm typecheck`=0, 107 tests (greeting, top-N newest-first + vide, routage Find/Join/récents, a11y), **navigateur réel** (nom « Othmane » + 3 récents riches, aucun « Maya » statique, Find→/invite, Join→/join, récents→/history).*
- [x] 2.9 — History enrichi & fiable — *Livré (local AsyncStorage) : schéma riche `{ id, peerLabel, startedAt, endedAt, durationMs, outcome }` ; hook moteur enregistre **une** entrée sur **chaque** terminal (met/abandoned/expired via `ended`/`state`, lost via reconnexions épuisées) avec dédup + `startedAt` capturé à l'état actif ; `/history` rend des lignes riches (nom, temps relatif, durée, badge d'issue) **newest-first** + état vide ; entrées legacy/partielles normalisées sans crash ; `add` ne clobber plus l'historique persisté quand le store n'est pas hydraté. Preuves : `pnpm typecheck`=0, 102 tests (mappings d'outcome, recordTerminal par outcome, écran riche/legacy/ordre, helpers de format ; tests 2.5 étendus restent verts), **navigateur réel** (/history seedé riche + nouvelle entrée correcte « Left » après une fin de session simulée via `/cancel`).*
- [x] 2.8 — Écran de diagnostic de proximité (outil dev) — *Livré : constantes tunables extraites en `proximityConfig` mutable (**défauts identiques** — rien changé), lu par le **même** `ProximityPipeline` (aucune logique parallèle) ; route `/diagnostics` (lien depuis Profil) : readouts live (brut/lissé/zone/tendance/lock+count/tier), **sparkline SVG maison** (`react-native-svg`, pas de lib de charting), sliders de tuning live, injection de RSSI. Preuves : `pnpm typecheck`=0, 87 tests (config se propage à computeProximity+orchestrateur, défauts inchangés → 2.2/2.3 verts, intégration live OK), **navigateur réel** — injection → readouts + 2 polylines, tuning `very_close -65→-63` → zone `very_close→close` pour le même signal ; `expo start --web` boot + `/diagnostics` atteignable.*
- [x] 2.7 — Polish & durcissement (pas de feature) — *Livré : erreurs typées (`ApiError` + mapping 403/404/409/410/réseau), flow avec phase de chargement + erreur (pas de hang, pas de double-submit), WS avec plafond de reconnexions + signal d'épuisement → état « connexion perdue », écran **permission refusée** (explication + réglages + réessayer), a11y du repérage (signal non-couleur par tier + labels lecteur d'écran + cible ✕ 44px), validation code/nom. **Sécurité : `joinUrl` met la capability uniquement dans le fragment** (`#cap=`), parseur la lit du fragment, body POST inchangé. Modif backend = uniquement la construction du joinUrl. Preuves : `pnpm typecheck`=0, 78 tests (échec/sécurité/a11y/chargement) + intégration live (joinUrl fragment-only, `/join` marche), `expo start --web` boot + flow (9 routes en 200).*
